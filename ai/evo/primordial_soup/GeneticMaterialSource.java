...
//    }
    ...
    //--------------------------------------------------------------------
//    public <T> IoGene<T> addIo(T isTrue)
//    {
//        IoGene<T> ioMaterial =
//                new IoGene<T>(isTrue);
//        return ioMaterial;
//    }
    ...
    public GenePool addMethods(Class clazz, AffinityDomain domain)
    {
        ...
        boolean defaultExclude = clazz.isAnnotationPresent(Exclude.class);
        for (Method method : clazz.getMethods())
        {
            if (! isAcceptable(method)) continue;
            if (method.isAnnotationPresent(Exclude.class)) continue;
            if (method.isAnnotationPresent(Include.class) ||
                    !defaultExclude)
            {
                addGenericMethod(fastClass, methods, method, domain);
            }
        }
    ...
    {
        Message msg    = new MethodMessage(fastClass, method);
            ...
            List<Affinity> args)
    {
        ...
        to.add( new GeneticMaterial(message.asCoding(), promoter) );
    }
