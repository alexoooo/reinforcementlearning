...
public abstract class AbstractDeme implements Deme
{
    ...
    //--------------------------------------------------------------------
    @Inject public void injectPopulationSize(
    ...
    //--------------------------------------------------------------------
    public void replace(Genome genome, Genome with)
    {
        if (pop.remove(genome))
        {
            pop.add( with );
        }
    }
    ...
    final public void select(Feedback pressure, PrimordialSoup soup)
    {
        ...
        for (Genome nonActor : nonActors)
        {
            ...
            {
                replace(nonActor,
                        (Genome) highQualityRandomIndividual().replicate());
            }
            else
            {
    ...
    public Genome highQualityRandomIndividual()
    {
    ...
    @SuppressWarnings("unchecked")
    public Genome lowQualityRandomIndividual()
    {
        if (pop.isEmpty()) return null;

        Genome crappy = Rand.fromList( pop );
        for (int i = 0; i < 16; i++)
        {
            Genome random = Rand.fromList( pop );
            if (crappy.compareTo(random) > 0)
            {
                crappy = random;
            }
        }
    ...
    {
        Genome genomeA = pop.get(indexA);
        Genome genomeB = pop.get(indexB);
        int fitnessCmp = genomeA.compareTo( genomeB );

        if (fitnessCmp == 0)
        {
            int sizeCmp = genomeA.size() - genomeB.size();
            return (sizeCmp < 0)
