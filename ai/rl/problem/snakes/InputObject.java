...
public class InputObject
{
    ...
    private final List<Coord> MY_JOINTS  = new ArrayList<Coord>();
    private final List<Vec>   OPP_BODY   = new ArrayList<Vec>();
    private final List<Coord> OPP_JOINTS = new ArrayList<Coord>();
                       ...
                       Snake            opponent)
    {
        ...
        for (int i = 0; i < howMany; i++)
        {
            ...
            MY_JOINTS.add( toCoord(myFrom) );
            OPP_JOINTS.add( toCoord(oppFrom) );

            MY_BODY.add(new Vec(new Num(myTo.getColumn() - myFrom.getColumn()),
                                new Num(myFrom.getRow()  - myTo.getRow())) );
            OPP_BODY.add(new Vec(new Num(oppTo.getColumn() - oppFrom.getColumn()),
                                 new Num(oppFrom.getRow()  - oppTo.getRow())) );
        }
    }
    ...
    public Coord takenAt(Coord from, Num angle)
    {
        ...
        while (isEmpty(from.plus(cursor)).isTrue())
        {
    ...
    //--------------------------------------------------------------------
    // for sensing your, and the opponent's body.  tail is 0.

    public Coord myJoin0()       { return MY_JOINTS.get(0); }
    public Coord myJoin1()       { return MY_JOINTS.get(1); }
    public Coord myJoin2()       { return MY_JOINTS.get(2); }
    public Coord myJoin3()       { return MY_JOINTS.get(3); }
    public Coord myJoin4()       { return MY_JOINTS.get(4); }
    public Vec   myVertebrae01() { return MY_BODY.get(0);   }
    public Vec   myVertebrae12() { return MY_BODY.get(1);   }
    public Vec   myVertebrae23() { return MY_BODY.get(2);   }
    public Vec   myVertebrae34() { return MY_BODY.get(3);   }
    public Vec   myVertebrae45() { return MY_BODY.get(4);   }

    public Coord oppJoin0()       { return OPP_JOINTS.get(0); }
    public Coord oppJoin1()       { return OPP_JOINTS.get(1); }
    public Coord oppJoin2()       { return OPP_JOINTS.get(2); }
    public Coord oppJoin3()       { return OPP_JOINTS.get(3); }
    public Coord oppJoin4()       { return OPP_JOINTS.get(4); }
    public Vec   oppVertebrae01() { return OPP_BODY.get(0);   }
    public Vec   oppVertebrae12() { return OPP_BODY.get(1);   }
    public Vec   oppVertebrae23() { return OPP_BODY.get(2);   }
    public Vec   oppVertebrae34() { return OPP_BODY.get(3);   }
    public Vec   oppVertebrae45() { return OPP_BODY.get(4);   }


    //----------------------------------------------------------------
    private BoardLocation toLocation(Coord coord)
    {
        long deltaRows = Math.round(coord.y().value());
        long deltaCols = Math.round(coord.x().value());
